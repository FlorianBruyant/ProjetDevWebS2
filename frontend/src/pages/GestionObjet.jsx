...
import { 
    ArrowBack, Save, Delete, Lock, Build, 
    History, Speed, Traffic, LocalParking, EventNote 
    ...
    return (
        <Box sx={{ bgcolor: '#F4F6F8', minHeight: '100vh', py: 4, mt: 8 }}>
                ...
                <Paper elevation={4} sx={{ p: 4, borderRadius: 4 }}>
                    ...
                    {tabActif === 0 && (
                        <Grid container spacing={3}>
                            ...
                            <Grid item xs={12} sm={6}>
                                <FormControlLabel
                                    control={<Switch checked={formData.est_actif} disabled={!isAdmin} onChange={(e) => setFormData({...formData, est_actif: e.target.checked})} />}
                                    label="Équipement sous tension"
                                />
                            </Grid>
                            <Grid item xs={12} sm={6}>
                                <FormControlLabel
                                    control={<Switch checked={formData.en_panne} disabled={!isAdmin} onChange={(e) => setFormData({...formData, en_panne: e.target.checked})} color="error" />}
                            ...
                            {isAdmin && (
                                <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'space-between', mt: 3 }}>
                                    <Button color="error" variant="text" startIcon={<Delete />} onClick={() => setOpenDeleteModal(true)}>Supprimer l'unité</Button>
                                ...
                                {objet?.historique && objet.historique.length > 0 ? (
                                    objet.historique.map((log, index) => (
                                        <ListItem key={index} divider={index !== objet.historique.length - 1}>
                                            <ListItemText 
                                                primary={log.action} 
                                                secondary={new Date(log.date_action).toLocaleString('fr-FR')} 
                                            />
                                            {log.points_gagnes > 0 && (
            ...
            <Dialog open={openDeleteModal} onClose={() => setOpenDeleteModal(false)}>
                ...
                <DialogContent>
                    <DialogContentText>Cette action supprimera définitivement l'objet <strong>{objet?.nom}</strong> de la base de données urbaine.</DialogContentText>
                </DialogContent>
                <DialogActions sx={{ p: 3 }}>
                    <Button onClick={() => setOpenDeleteModal(false)}>Annuler</Button>
                    <Button color="error" variant="contained" onClick={async () => {
                        const token = localStorage.getItem('access_token');
                        await fetch(`http://localhost:8000/api/map/${type_api}/${id}/`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
                        navigate('/carte');
